...
// INTEGRAÇÃO DOS EVENTOS NA INICIALIZAÇÃO DA DOM
...
// GERAÇÃO DINÂMICA DO LAYOUT DOS TECLADOS SEPARADOS
function renderKeyboardLayouts() {
    ...
    notes.forEach(n => {
        notesRow.innerHTML += `<button class="btn-key" onclick="answerPitch('${n}')">${n}</button>`;
    });
    
    pausas.forEach(p => {
        pausesRow.innerHTML += `<button class="btn-key btn-pause" onclick="answerPitch('${p}')">${NAMES[p]}</button>`;
    });
    
    tempos.forEach(t => {
        timeRow.innerHTML += `<button class="btn-key btn-time-key" onclick="answerTime('${t}')">${t}</button>`;
    });
}
...
function generateSeq() {
    ...
    while (rem > 0) {
        ...
        if (!opts.length) {
            const fallback = ['semibreve', 'minima', 'seminima', 'colcheia', 'semicolcheia'];
            opts = fallback.filter(f => getTicks(f) === rem);
            if (!opts.length) break;
        }
        
        const f = opts[Math.floor(Math.random() * opts.length)];
        figs.push(f); rem -= getTicks(f);
    }
    ...
    for (let i = 0; i < figs.length; i++) {
        const fig = figs[i];
        const xPct = figs.length === 1 ? ((xStart + xEnd) / 2) + '%' : (xStart + step * i) + '%';
        ...
        if (isPause) {
            ...
            d.className = 'pausa-texto'; d.style.left = xPct; d.innerHTML = PAUSE_SYMS[fig];
            if (fig === 'pausa_semibreve') d.style.bottom = '44px';
            else if (fig === 'pausa_minima') d.style.bottom = '54px';
            else d.style.bottom = '48px';
            staff.appendChild(d);
        } else {
            const w = document.createElement('div');
            w.className = 'nota-wrapper'; w.style.left = xPct; w.style.bottom = pitch.y + 'px';
            ...
            if (fig !== 'semibreve') {
                ...
                st.className = isUp ? 'stem-up' : 'stem-down'; w.appendChild(st);

                if (fig === 'colcheia' || fig === 'semicolcheia') {
                    const f1 = document.createElement('div');
                    f1.className = isUp ? 'flag-colcheia-up' : 'flag-colcheia-down'; w.appendChild(f1);
                    if (fig === 'semicolcheia') {
                        const f2 = document.createElement('div');
                        f2.className = isUp ? 'flag-semi-up' : 'flag-semi-down'; w.appendChild(f2);
                    }
                }
            }
            staff.appendChild(w);
    ...
    activeIndex = 0; activeType = 'pitch';
    renderSlots();
}
...
function drawSupp(y, x) {
    ...
    const add = (yy) => {
        ...
        l.className = 'linha-suplementar'; l.style.bottom = yy + 'px'; l.style.left = x;
        staff.appendChild(l);
    };
    if (y >= 90) add(100);
    if (y <= -10) add(-20);
...
function renderSlots() {
    ...
    for (let i = 0; i < sequence.length; i++) {
        ...
        col.appendChild(sp); col.appendChild(st); cont.appendChild(col);
    }
    refreshSlots();
}
...
function answerPitch(val) {
    if (isAnimating) return;
    sfxClick();
    answersPitch[activeIndex] = val; slotStatusPitch[activeIndex] = '';
    const s = document.getElementById('slot-p-' + activeIndex);
    s.classList.add('pop'); setTimeout(() => s.classList.remove('pop'), 300);
    activeType = 'time';
    document.getElementById('message-area').innerHTML = ''; refreshSlots();
}

function answerTime(val) {
    if (isAnimating) return;
    sfxClick();
    answersTime[activeIndex] = val; slotStatusTime[activeIndex] = '';
...
function refreshSlots() {
    for (let i = 0; i < sequence.length; i++) {
        ...
        sp.innerHTML = vp ? (vp.includes('pausa') ? `<span style="font-family:'Noto Music',serif;font-size:26px;display:inline-block;transform:translateY(-2px)">${PAUSE_SYMS[vp]}</span>` : `<span>${vp.toUpperCase()}</span>`) : '';
...
function verify() {
    ...
    for (let i = 0; i < sequence.length; i++) {
        let pGiven = answersPitch[i], pCorrect = sequence[i].answerPitch;
        let tGiven = answersTime[i], tCorrect = sequence[i].answerTime;
        let pOk = (pGiven === pCorrect), tOk = (tGiven === tCorrect);

        slotStatusPitch[i] = pOk ? 'correct' : 'wrong';
        slotStatusTime[i] = tOk ? 'correct' : 'wrong';

        if (pOk && tOk) { if (!slotEvaluated[i]) { totalRight++; slotEvaluated[i] = true; } } 
        else {
            allRight = false; if (!slotEvaluated[i]) { totalWrong++; slotEvaluated[i] = true; }
            let txtNoteCorrect = NAMES[pCorrect] || pCorrect;
            if (!pOk && !tOk) errHTML += `<div class="msg-error-item">❌ <b>Caixa ${i + 1}:</b> Nota e Tempo errados. Era <b>${txtNoteCorrect}</b> valendo <b>${tCorrect}</b>.</div>`;
            else if (!pOk) errHTML += `<div class="msg-error-item">❌ <b>Caixa ${i + 1}:</b> Nota errada. Era <b>${txtNoteCorrect}</b>.</div>`;
            else if (!tOk) errHTML += `<div class="msg-error-item">❌ <b>Caixa ${i + 1}:</b> Tempo errado. Era <b>${tCorrect}</b>.</div>`;
        }
    }
    ...
    if (allRight) {
        setInteractionState(false); streak++; sfxOk(); updateHeader();
        let html = '<div class="msg-success">🎉 Perfeito! Acertou notas e tempos!</div>';
        if (streak >= 3 && level < 6) { level++; streak = 0; sfxUp(); updateHeader(); html += `<div class="msg-level-up">🚀 Avançou para o Nível ${level}!</div>`; confetti(); } 
        else if (level === 6 && streak >= 3) { streak = 0; sfxUp(); updateHeader(); html += `<div class="msg-level-up">🏆 Mestre! Você dominou tudo!</div>`; confetti(); }
...
function endSession() {
    clearTimeout(nextTimer); setInteractionState(true); const total = totalRight + totalWrong;
    if (total === 0) {
        document.getElementById('summary-emoji').textContent = '🤷'; document.getElementById('summary-title').textContent = 'Nenhuma nota avaliada';
        document.getElementById('summary-sub').textContent = 'Tente verificar pelo menos uma resposta antes de terminar.';
        document.getElementById('summary-bar').style.width = '0%'; showScreen('screen-summary'); return;
    }
    ...
    document.getElementById('summary-emoji').textContent = emoji; document.getElementById('summary-title').textContent = title; document.getElementById('summary-sub').textContent = sub;
    document.getElementById('stat-acertos').textContent = totalRight; document.getElementById('stat-erros').textContent = totalWrong;
    document.getElementById('stat-pct-a').textContent = pct + '%'; document.getElementById('stat-pct-e').textContent = (100 - pct) + '%';
    showScreen('screen-summary'); setTimeout(() => { document.getElementById('summary-bar').style.width = pct + '%' }, 100);
    if (pct >= 70) sfxUp();
}
...
function confetti() {
    ...
    for (let i = 0; i < 40; i++) {
        setTimeout(() => {
            const p = document.createElement('div'); p.className = 'confetti-piece'; p.style.left = (Math.random() * 100) + 'vw';
            p.style.top = '-10px'; p.style.background = colors[Math.floor(Math.random() * colors.length)];
            p.style.animationDuration = (1 + Math.random() * .8) + 's'; p.style.animationDelay = (Math.random() * .4) + 's';
